...
5. summarizes entrainment candidates, envelope modulation, and phase behavior for
   each population; and
6. masks nonmatching windows and rebuilds continuity-qualified protocol tracks so
   gaps between speech contexts cannot be mistaken for persistence.
...
Envelope and phase measurements are restricted to the selected carrier pair's
actual support interval. Carrier selection prefers the longest-supported eligible
pair, and simple integer harmonics such as an octave are explicitly labeled and
penalized rather than treated as ordinary binaural candidates.

## Interpretation

A persistent signature that remains similar in speech-active and speech-sparse windows is less
